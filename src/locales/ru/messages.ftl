...
draft-no-project-btn = ➖ Без проекта
draft-no-project = (без проекта)
draft-choose-team = Выберите команду:
...
due-prompt = Отправьте дату в формате ДД.ММ.ГГГГ (например: 30.06.2026).
due-invalid = Не понял дату. Формат: ДД.ММ.ГГГГ (например: 30.06.2026).
...
people-current = Назначенные задачи:
...
admin-remove-user = 🗑 Удалить пользователя

rmuser-choose = Кого удалить из бота?
rmuser-confirm = Удалить { $name } из бота? Его роли, участие в проектах и подписки будут удалены. Действие необратимо.
rmuser-yes = 🗑 Удалить
rmuser-done = 🗑 { $name } удалён из бота.
